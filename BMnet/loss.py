...
def get_loss(cfg, device):
    ...
    if cfg.TRAIN.counting_loss == 'l1loss':
        counting_loss = L1Loss(device = device, reduction='mean',
                               downsampling_rate=cfg.DATASET.downsampling_rate)
    elif cfg.TRAIN.counting_loss == 'l2loss':
        counting_loss = L2Loss(device = device, reduction='mean',
                               downsampling_rate=cfg.DATASET.downsampling_rate)
    else:
    ...
    if cfg.TRAIN.contrast_loss == 'info_nce':
        contrast_loss = InfoNCELoss(device = device, downsampling_rate=feature_drate)
