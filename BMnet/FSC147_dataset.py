...
class FSCD_LVISDataset(Dataset):
    def __init__(self, data_path, split, scaling, box_number=3, scale_number=20, min_size=384, max_size=1584,
    ...
    def __getitem__(self, idx):
        ...
        if file_name in self.images:
            ...
            image_path = os.path.join(self.img_root_path, file_name)
            density_path = os.path.join(self.density_root_path, file_name.replace('jpg', 'npy'))
            ...
            for bbox in ex_bboxes[:3]:
                ...
                x2, y2 = x + w, y + h
                x3, y3 = x, y + h
                temp_res = [[x0, y0], [x1, y1], [x2, y2], [x3, y3]]
                rects.append(temp_res)
            ...
            # target = np.zeros((nh, nw), dtype=np.float32)
            density_map = np.load(density_path).astype(np.float32)
            resized_density = cv2.resize(density_map, (nw, nh))
            orig_count = np.sum(density_map)
            new_count = np.sum(resized_density)
            if new_count > 0: resized_density = resized_density * (orig_count / new_count)

            pt_map = np.zeros((nh, nw), dtype=np.int32)
            points = (np.array(self.count_anno["annotations"][idx]["points"]) * r).astype(np.int32)
            boxes = boxes * r
            boxes = boxes[:self.box_number, :, :]
            gtcount = points.shape[0]
            # crop patches and data transformation
            ...
            target['density_map'] = resized_density * self.scaling
            target['pt_map'] = pt_map
            target['gtcount'] = gtcount
            target['scale_embedding'] = torch.tensor(scale_embedding)
            img, target = self.main_transform(img, target)
...
def Int_build_dataset(cfg):
    main_transform = MainTransform()
    query_transform = get_query_transforms(False, cfg.DATASET.exemplar_size)
    val_data_list = cfg.DATASET.list_val
    test_data_list = cfg.DATASET.list_test

    val_dataset = FSC147Dataset(data_dir=cfg.DIR.dataset,
    ...
    test_dataset = FSC147Dataset(data_dir=cfg.DIR.dataset,
...
def Build_FSCD_LVIS(cfg):
    main_transform = MainTransform()
    query_transform = get_query_transforms(False, cfg.DATASET.exemplar_size)

    train_dataset = FSCD_LVISDataset(data_path=cfg.DIR.dataset,
                                   split='train',
                                   scaling=1.0,
                                   box_number=cfg.DATASET.exemplar_number,
                                   scale_number=cfg.MODEL.ep_scale_number,
    ...
    val_dataset = FSCD_LVISDataset(data_path =cfg.DIR.dataset,
                            split='val',
                            scaling=1.0,
                            box_number=cfg.DATASET.exemplar_number,
                            scale_number=cfg.MODEL.ep_scale_number,
    ...
    test_dataset = FSCD_LVISDataset(data_path =cfg.DIR.dataset,
                                split='test',
                                scaling=1.0,
                                box_number=cfg.DATASET.exemplar_number,
                                scale_number=cfg.MODEL.ep_scale_number,
